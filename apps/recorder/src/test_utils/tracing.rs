...
pub fn try_init_testing_tracing(level: Level) {
  let _ = tracing_subscriber::fmt().with_env_filter(build_testing_tracing_filter(level)).try_init();
}
...
// NOTE: Gated behind feature flag because `tracing_test` crate is not
// currently in Cargo.toml. Re-enable when the dependency is added.
#[cfg(feature = "tracing-test")]
mod traced_test {
...
#[cfg(feature = "tracing-test")]
pub use traced_test::{logs_contain, setup_traced_test};
