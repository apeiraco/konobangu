...
use crate::{
  ...
  graphql::{
    ...
    infra::{
      custom::{
        generate_entity_default_basic_entity_object, generate_entity_default_insert_input_object, generate_entity_filtered_mutation_field,
        register_entity_default_readonly,
      },
      json::{convert_jsonb_output_for_entity, restrict_jsonb_filter_input_for_entity},
      name::{
        GraphqlColumnKey, get_entity_basic_type_name, get_entity_create_one_mutation_field_name, get_entity_custom_mutation_field_name,
        get_entity_delete_mutation_field_name, get_entity_insert_input_type_name, get_entity_renormalized_data_field_name,
      },
    },
  },
  ...
  <T as EntityTrait>::Model: Sync,
{
  ...
  let options = context.types.column_options.entry(entity_column_id.clone()).or_default();

  options.input_type = Some(TypeRef::Named(system_tasks::SystemTask::ident(&ts_rs::Config::default()).into()));
  options.output_type = Some(TypeRef::Named(system_tasks::SystemTask::ident(&ts_rs::Config::default()).into()));
  ...
  options.input_conversion = Some(Arc::new(
    move |value_accessor: &async_graphql::dynamic::ValueAccessor| -> seaography::SeaResult<sea_orm::Value> {
      ...
      let json_value =
        serde_json::to_value(task).map_err(|err| SeaographyError::TypeConversionError(err.to_string(), format!("Json - {entity_column_name}")))?;
...
pub fn register_system_tasks_to_schema_builder(mut builder: SeaographyBuilder) -> SeaographyBuilder {
  builder.schema = builder
    .schema
    .register(Scalar::new(system_tasks::SystemTask::ident(&ts_rs::Config::default())).description(system_tasks::SystemTask::decl(&ts_rs::Config::default())));
  ...
  {
    let entity_retry_one_mutation_name = get_entity_custom_mutation_field_name::<system_tasks::Entity>(builder_context, "RetryOne");
    let retry_one_mutation = generate_entity_filtered_mutation_field::<system_tasks::Entity, _, _>(
      builder_context,
  ...
  }
  {
      ...
      move |resolve_context| {
        FieldFuture::new(async move {
          let guard_result = builder_context.hooks.entity_guard(
            &resolve_context,
            &crate::graphql::infra::name::get_entity_name::<system_tasks::Entity>(builder_context),
            seaography::OperationType::Create,
          );
          if let seaography::GuardAction::Block(reason) = guard_result {
            return Err(async_graphql::Error::new(reason.unwrap_or("Entity guard triggered.".into())));
          }
    ...
    .argument(InputValue::new(
      ...
      TypeRef::named_nn(get_entity_insert_input_type_name::<system_tasks::Entity>(builder_context)),
    ));
